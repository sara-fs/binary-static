...
const getPropertyValue = require("../../../../../_common/utility")
  .getPropertyValue;
...
const PersonalDetails = (() => {
  ...
  const init = () => {
    ...
    is_mt_tax_required =
      /real/.test(mt_acct_type) &&
      mt_acct_type.split("_").length > 2 &&
      +State.getResponse("landing_company.config.tax_details_required") === 1;
  };
  ...
  const shouldShowTax = (get_settings) => {
    ...
    const has_set_tax =
      get_settings.tax_identification_number || get_settings.tax_residence;
    return is_tax_req || has_set_tax;
  };

  const showHideMissingDetails = () => {
    const has_missing_field = validations.find(
      (validation) =>
        /req/.test(validation.validations) &&
        $(validation.selector).val() === ""
    );
  ...
  const displayChangeableFields = (get_settings) => {
    ...
    const is_changeable_citizen = !get_settings.immutable_fields.includes(
      "citizen"
    );
    const is_changeable_pob = !get_settings.immutable_fields.includes(
      "place_of_birth"
    );
    if (is_changeable_pob || is_changeable_citizen) {
      ...
      residence_list.forEach((res) => {
        $options.append(
          CommonFunctions.makeOption({ text: res.text, value: res.value })
        );
      });
      $options.prepend(
  ...
  const getDetailsResponse = (
    data,
    residence_list = State.getResponse("residence_list")
  ) => {
    const get_settings = $.extend({}, data);
    // date_of_birth can be 0 as a valid epoch
    get_settings.date_of_birth =
      "date_of_birth" in get_settings && get_settings.date_of_birth !== "null"
        ? moment
    ...
    const hide_name =
      accounts.some((loginid) =>
        new RegExp(loginid, "i").test(get_settings.first_name)
      ) || is_virtual;
    if (!hide_name) {
      get_settings.name = `${get_settings.first_name || ""} ${
    ...
    if (
      ...
      get_settings.immutable_fields.includes("place_of_birth") &&
      residence_list
    ) {
      get_settings.place_of_birth =
        (
          residence_list.find(
            (obj) => obj.value === get_settings.place_of_birth
          ) || {}
    ...
    if (
      ...
      get_settings.immutable_fields.includes("citizen") &&
      residence_list
    ) {
      get_settings.citizen =
        (residence_list.find((obj) => obj.value === get_settings.citizen) || {})
    ...
    if (is_virtual) {
      ...
      showHideTaxMessage();
      CommonFunctions.getElementById("tax_information_form").setVisibility(
        shouldShowTax(get_settings)
      );
  ...
  const displayGetSettingsData = (get_settings) => {
    Object.keys(get_settings).forEach((key) => {
      ...
      editable_fields[key] =
        get_settings[key] !== null ? get_settings[key] : "";
      ...
      if (element_key.type === "checkbox") {
        ...
        const getOptionText = (value) =>
          (document.querySelector(`#${key} option[value="${value}"]`) || {})
            .innerText || value;
        let localized_text;
        if (key === "tax_residence") {
          // Resolve comma-separated country codes to country names
          localized_text = get_settings[key]
            ? get_settings[key]
                .split(",")
            ...
            if (this.getAttribute("id") === "date_of_birth") {
              ...
              const date_of_birth =
                this.value.indexOf("-") < 0
                  ? toISOFormat(moment(this.value, "DD MMM, YYYY"))
                  : this.value;
    ...
    if (get_settings.country) {
      $("#residence").replaceWith(
        $("<label/>").append($("<strong/>", { id: "country" }))
      );
    ...
    if (is_virtual) {
      ...
      validations = [
        ...
        { selector: "#address_city", validations: ["req", "letter_symbol"] },
        {
          ...
          validations:
            $("#address_state").prop("nodeName") === "SELECT"
              ? ""
              : ["letter_symbol"],
        },
        {
          ...
          validations: [
            residence === "gb" ||
            Client.get("landing_company_shortcode") === "iom"
              ? "req"
              : "",
            "postcode",
            ["length", { min: 0, max: 20 }],
        ...
        // recheck tax_identiciation_number after tax_residence is selected as the validation regex is taken from API based on tax residence
        {
          ...
          validations: is_tax_req ? ["req"] : "",
          re_check_field: "#tax_identification_number",
        },
        {
        ...
        {
          selector: "#citizen",
          validations: is_financial || is_gaming || mt_acct_type ? ["req"] : "",
        },
        {
          selector: "#chk_tax_id",
          validations: is_financial
            ? [
                [
                  "req",
                  {
      ...
      for (let i = validations.length - 1; i >= 0; i--) {
        ...
        if (
          !validations[i].exclude_request &&
          get_settings_data.immutable_fields.includes(
            validations[i].selector.slice(1)
          )
        ) {
  ...
  const setDetailsResponse = (response) => {
    ...
    if (!is_error) {
      ...
      // is_error
      showFormMessage(
        getPropertyValue(response, ["error", "message"]) ||
          localize("Sorry, an error occurred while processing your account."),
        false
      );
    }
  };
  ...
  const showFormMessage = (localized_text, is_success) => {
    const $ul = $("<ul/>", { class: "checked" }).append(
      $("<li/>", { text: localized_text })
    );
    $("#formMessage")
      ...
      if (residence_list.length > 0) {
        ...
        if (residence) {
          if (shouldShowTax(get_settings_data)) {
            $tax_residence = $("#tax_residence");
            $tax_residence
              .html($options_with_disabled.html())
              .promise()
              .done(() => {
                setTimeout(() => {
                  const residence_value = get_settings_data.tax_residence
                    ? get_settings_data.tax_residence.split(",")
                    : residence || "";
                  $tax_residence
                    .select2()
                    .val(residence_value)
          ...
          $("#country")
            .parent()
            .replaceWith($("<select/>", { id: "residence", single: "single" }));
          const $residence = $("#residence");
          $options_with_disabled.prepend(
            $("<option/>", {
              text: localize("Please select a country"),
              value: "",
            })
          );
  ...
  const displayResidenceList = () => {
    BinarySocket.send({ residence_list: 1 }).then((response) => {
      populateResidence(response).then(() => {
        if (residence) {
          BinarySocket.send({ states_list: residence }).then(
            (response_state) => {
